// Wraps an architecture-specific single-threaded implementation of the PdfiumLibraryBindings
// trait behind a mutex marshaller to create a thread-safe trait implementation.
//
// Pdfium itself is not thread-safe: its C API is non-reentrant, so concurrent calls into the
// library from different threads corrupt its internal state. This wrapper acquires a single
// process-wide mutex at the top of EVERY bindings method and holds it for the duration of that
// FFI call, then forwards to the inner single-threaded bindings. Serialising every call is the
// only way to soundly share one Pdfium instance across threads, which is why `Pdfium` is marked
// `Send + Sync` only when the `thread_safe` feature wraps its bindings in this type.
//
// The lock lives INSIDE the bindings, not at pdfium-render's call sites, so it cannot be
// bypassed: any code holding a `&dyn PdfiumLibraryBindings` (including callers reaching the raw
// bindings through `Pdfium::bindings()`) is serialised by construction. See issue #262.
//
// This file is generated to mirror the `PdfiumLibraryBindings` trait exactly: every abstract
// (FFI leaf) method is overridden to lock-and-forward. Default trait methods compose these
// leaves and so are serialised transitively; they are intentionally not overridden.
...
impl PdfiumThreadMarshall {
    ...
    /// Returns exclusive access to the global [PdfiumThreadMarshall] singleton.
...
// The PdfiumLibraryBindings trait requires `Drop`; every bindings implementation provides one so
// that a bindings box which never becomes the process-global instance is torn down correctly.
// The wrapper simply drops its inner bindings (running their Drop) when it is itself dropped.
...
impl<T: PdfiumLibraryBindings> PdfiumLibraryBindings for ThreadSafePdfiumBindings<T> {
    ...
    unsafe fn FPDF_StructElement_GetExpansion(
        ...
        let _lock = PdfiumThreadMarshall::lock();
        self.bindings
            .FPDF_StructElement_GetExpansion(struct_element, buffer, buflen)
    }
    ...
    unsafe fn FPDFDOC_ExitFormFillEnvironment(&self, form: FPDF_FORMHANDLE) {
        let _lock = PdfiumThreadMarshall::lock();
        self.bindings.FPDFDOC_ExitFormFillEnvironment(form)
    }
    ...
    unsafe fn FORM_OnAfterLoadPage(&self, page: FPDF_PAGE, form: FPDF_FORMHANDLE) {
        let _lock = PdfiumThreadMarshall::lock();
        self.bindings.FORM_OnAfterLoadPage(page, form)
    }
    ...
    unsafe fn FORM_OnBeforeClosePage(&self, page: FPDF_PAGE, form: FPDF_FORMHANDLE) {
        let _lock = PdfiumThreadMarshall::lock();
        self.bindings.FORM_OnBeforeClosePage(page, form)
    }
    ...
    unsafe fn FPDF_SetSystemFontInfo(&self, font_info: *mut FPDF_SYSFONTINFO) {
        let _lock = PdfiumThreadMarshall::lock();
        self.bindings.FPDF_SetSystemFontInfo(font_info)
    }
    ...
    unsafe fn FPDF_FreeDefaultSystemFontInfo(&self, font_info: *mut FPDF_SYSFONTINFO) {
        let _lock = PdfiumThreadMarshall::lock();
        self.bindings.FPDF_FreeDefaultSystemFontInfo(font_info)
    }
    ...
    unsafe fn FPDFTextObj_SetFontSize(&self, text: FPDF_PAGEOBJECT, size: c_float) -> FPDF_BOOL {
        let _lock = PdfiumThreadMarshall::lock();
        self.bindings.FPDFTextObj_SetFontSize(text, size)
    }
    ...
    unsafe fn FPDFText_SetPositions(
        ...
        let _lock = PdfiumThreadMarshall::lock();
        self.bindings
            .FPDFText_SetPositions(text_object, positions, count)
    }
    ...
    unsafe fn FPDFPageObj_AddExistingMark(
        ...
        let _lock = PdfiumThreadMarshall::lock();
        self.bindings.FPDFPageObj_AddExistingMark(page_object, mark)
    }
    ...
    unsafe fn FPDFCatalog_GetLanguage(
        ...
        let _lock = PdfiumThreadMarshall::lock();
        self.bindings
            .FPDFCatalog_GetLanguage(document, buffer, buflen)
    }
...
mod tests {
    ...
    use crate::bindings::dynamic_bindings::DynamicPdfiumBindings;
    use crate::bindings::PdfiumLibraryBindings;
    use std::path::Path;
    ...
    // marshall mutex is *free* once any bindings call has returned. An earlier
    // model acquired the mutex inside FPDF_InitLibrary and parked the guard on
    // the wrapper, so the mutex stayed locked for the lifetime of the wrapper
    // and every later call bypassed synchronisation. This test drives
    // FPDF_InitLibrary and then checks that the marshall mutex can still be
    // taken -- deterministically, with no reliance on a data race.
    //
    ...
    fn marshall_mutex_is_released_after_call_returns() {
        ...
        // we found it.
        unsafe { bindings.FPDF_DestroyLibrary() };
    }
}
